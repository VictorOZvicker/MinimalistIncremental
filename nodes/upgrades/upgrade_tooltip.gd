...
func set_labels(_definition: Upgrade, _upgrade_name: String):
	...
	var ident = "%*s" % [13, ""]
	var currency = "$ " if _definition.currency == Enums.UpgradeCostTags.MONEY else "P "
	...
	$upgradeInfos/upgradeCost.text        = ident + currency + str(Game.get_player().get_upgrade_cost(_upgrade_name))
