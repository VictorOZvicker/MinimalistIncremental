...
## uid do GeneratorInventory), ocupando space.x por space.y células.
## Arraste-a para movê-la dentro da grade ou para devolvê-la ao
...
func setup(_uid: int, _item: GeneratorItem, _cell: Vector2i, _source_grid: Control) -> void:
	...
	self.size = Vector2(item.space * CELL_SIZE)
